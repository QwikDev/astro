...
└── libs/
    └── qwikdev-astro/
        ...
        │   └── virtual.d.ts   # Virtual module types
...
- **`astro:config:setup`** — Registers the Qwik renderer via `addRenderer()`, defines the `virtual:qwik-astro` virtual module for runtime render options and the client manifest, resolves source/build paths, and configures `qwikVite()` with file filters, entry points, and output directories. In build mode, it strips Qwik's client-output hooks so the standalone client build remains authoritative.
...
The `virtual:qwik-astro` module exposes the manifest from the standalone client build and installs it on `globalThis` before the Qwik renderer runs. Development mode uses an empty manifest because it does not have production symbol mappings.
